        ...
        div(v-for="group in sidebar")
          ...
          .section(v-for="section in sortGroup(group.children)")
            ...
            div(v-if="active(section)")
              router-link(:to="item.path" tag="div" v-if="item.path" v-for="item in sortBy(section.children, ['frontmatter.order'])" :class="{'section__child__active': $page.key == item.key}").section__child {{item.title}}
      .footer
